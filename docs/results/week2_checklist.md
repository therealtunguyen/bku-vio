...
**Single source of truth:** [`table.md`](table.md). Mọi check ☑ dưới đây
phải đi kèm dòng tương ứng trong `table.md` với `Trạng thái` đã chuyển
sang `confirmed` (hoặc `smoke-test` cho HCMUT). Lệnh chạy ở
[`eval_commands.md`](eval_commands.md).
...
## B. Baseline EuRoC `V1_01_easy` (BẮT BUỘC — đóng paper với dòng này) ✅ ĐÃ XONG 2026-05-27

- [x] Chạy đúng block §A trong [`eval_commands.md`](eval_commands.md), `RUN_TAG=euroc_V1_01_easy`.
- [x] Kiểm log có dòng `Camera calibration: fx=458.654000, ...` và
      `Input sensor QoS reliability: reliable`.
- [x] Trajectory không diverge (kiểm bằng plot — `docs/paper/figures/euroc_V1_01_easy_baseline.png`).
- [x] `tools/evaluate_m4.py` ra 2 file `ape_aligned.zip`, `rpe_1m_aligned.zip` không lỗi.
- [x] Chạy `tools/plot_trajectory.py` ⇒ có `trajectory_xy.png` + `.pdf`.
- [x] Điền `ATE RMSE` (0.0905) và `RPE 1 m RMSE` (0.0949) vào dòng #1 trong [`table.md`](table.md),
      đổi trạng thái → `confirmed`.
- [x] Commit ảnh paper vào `docs/paper/figures/` (PNG/PDF không vào git ở `results/` vì gitignore).

## C. EuRoC — biến thể để trả lời "kết quả có tốt hơn không?"

> Mỗi biến thể là một dòng mới trong [`table.md`](table.md), **không ghi đè dòng baseline**.
...
Mỗi biến thể: lặp lại đầy đủ block §A của `eval_commands.md`, đổi `RUN_TAG` và override param tương ứng qua `--ros-args -p ...` (hoặc edit `vio_euroc.yaml` rồi rebuild).

## D. EuRoC — sequence khác (nếu thời gian cho phép, optional cho báo cáo CO3107)

- [ ] Tải `V1_02_medium`, `V2_01_easy` về `dataset/`.
- [ ] Tạo `RUN_TAG=euroc_V1_02_medium`, lặp §A với `dataset_dir` mới.
- [ ] Bổ sung dòng tương ứng vào [`table.md`](table.md).

> Nếu không có thời gian, **không bịa số cho các sequence chưa chạy**. Báo cáo chỉ ghi V1_01_easy.

## E. HCMUT smoke test (BẮT BUỘC — để xác nhận pipeline live) ✅ ĐÃ XONG 2026-05-27

- [x] Chạy đúng block §B trong [`eval_commands.md`](eval_commands.md), `RUN_TAG=hcmut_smoke`.
- [x] Kiểm log thấy `Camera calibration: fx=646.337..., ...` và `Input sensor QoS reliability: best_effort`.
- [x] Kiểm `/vio/odometry` có > 0 message → có 221 messages.
- [x] **Không** chạy `evaluate_m4.py`. **Không** điền số vào ATE/RPE.
- [x] Cập nhật ô `Ghi chú` của dòng #2: 221 poses, diverge sau ~12.6 s sim, max `‖pos‖` 8 836 m. Chi tiết: [`hcmut_smoke.md`](hcmut_smoke.md).
- [x] Trạng thái vẫn là `smoke-test`.

## F. Kiểm tra trước khi data-freeze (chiều 2026-05-30)

- [ ] Mỗi dòng `confirmed` trong [`table.md`](table.md) có đủ: lệnh, output dir tồn tại, ATE, RPE, commit hash.
- [ ] Mỗi dòng `smoke-test` ghi rõ **lý do không tính accuracy**.
- [ ] `table.md` không còn ô `_PENDING_` cho bất kỳ dòng nào sẽ vào báo cáo.
...
- [ ] Báo trong nhóm chat: "Đã freeze. Số liệu lấy từ `docs/results/table.md` @ tag `data-freeze-week2`."
...
- [ ] Khi cả 2 đủ ⇒ mở `RUN_TAG=hcmut_v1` mới, mở dòng mới trong [`table.md`](table.md), chạy `evaluate_m4.py` như EuRoC.

---

## Định nghĩa "Done" của Tuần 2

Nhóm có thể trả lời `kết quả có tốt hơn không?` chỉ bằng cách mở [`table.md`](table.md):
