...
def test_msckf_update_stats_leave_accepted_track_diagnostics_disabled_by_default():
    server = _make_server()
    updater = MSCKFUpdater(server)
    updater.distortion_coefficients = np.zeros(4)
    ...
    assert stats["rows"] > 0
    assert stats["accepted_track_diagnostics"] == []


def test_msckf_can_collect_accepted_track_diagnostics_when_enabled():
    server = _make_server()
    updater = MSCKFUpdater(server)
    updater.collect_feature_diagnostics = True
    updater.distortion_coefficients = np.zeros(4)
    point_w = np.array([0.4, -0.1, 4.5])
...
if __name__ == "__main__":
    ...
    test_msckf_update_stats_leave_accepted_track_diagnostics_disabled_by_default()
    test_msckf_can_collect_accepted_track_diagnostics_when_enabled()
